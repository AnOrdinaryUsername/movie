...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (result.ok) {
        const watchlist: MovieInfo[] = await result.json();
        setWatchlist(watchlist);
        setIsLoading(false);
      } else {
        setError('Unable to fetch watchlist. Please try again later.');
        setIsLoading(false);
      }
    })();
  }, []);
  ...
  async function removeMovie(event: MouseEvent<HTMLButtonElement>) {
    ...
    const { media_title } = watchlist.filter((movie) => movie.id === id)[0];
    ...
    const result = await fetch(`http://localhost:8000/api/v1/users/${user.id}/watchlist`, {
      method: 'DELETE',
    ...
    try {
      if (result.status !== 204) {
        throw new Error(
          `Unable to remove ${media_title} from watchlist. Please try again at a later time.`,
        );
      }
      ...
      notifications.show({
        ...
        autoClose: 30000,
        title: 'Deleted',
        message: `Removed ${media_title} from watchlist.`,
        color: 'yellow',
        loading: false,
      });
    } catch (error) {
